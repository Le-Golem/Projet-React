...
console.log(display)
    ...
    <h4 className="title-homepage-first">Séries populaires</h4>    <div className="slider-style-movie1">        {popularTvShows && popularTvShows.length > 0 && popularTvShows.map(movie => <img key={movie.id} src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} width="250" height="auto"/>)}    </div>    <h4 className="title-homepage-first">Séries les mieux notées</h4>    <div className="slider-style">        {topRatedTvShows && topRatedTvShows.length > 0 && topRatedTvShows.map(movie => <img key={movie.id} src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} width="250" height="auto"/>)}    </div>
        </div>}
